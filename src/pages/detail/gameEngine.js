...
export default class GameEngine {
  ...
  async getScript(progressId) {
    ...
    return {
      id: progressId,
      background: script.b ? `/common/bg/${script.b}.jpg` : "",
      // 有 CG/SD 图层时隐藏立绘
      character: (!script.cg && !script.sd && script.c) || "",  // c字段原值，由播放器解析为立绘图
      cg: script.cg ? `/common/evig/${script.cg}` : "",
      sd: script.sd ? `/common/evig/${script.sd}` : "",
      speaker: script.s || "",
      text: script.t || "",
      cs: script.cs || 100,
      fs: script.fs || 100,   // 文本字号缩放百分比(转换器已处理 %NN 标记)
      blur: script.blur || false,
      effect: script.e || false,
      flash: script.f || false,
      choose: !!script.co,
      choose1: script.c1 || "",
      choose2: script.c2 || "",
      choose3: script.c3 || "",
